...
if (!empty($action)) {
	...
	switch ($action) {
      ...
      $sql = 'insert into `'. PREFIX .'_info` ('. sqlBuilder($data, BUILDER_INSERT_NAME) .', rid, type, status, time) values ('. sqlBuilder($data, BUILDER_INSERT_VALUE) .', '. $id .', 3, '. $config['info'] .', '. time() .')';
